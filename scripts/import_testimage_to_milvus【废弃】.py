"""导入 testImage 图片到新 Milvus collection (MinIO URL)"""
import os
import sys
import re
import time
...
def create_collection() -> Collection:
    """创建新 collection"""
    print(f"[MILVUS] Checking collection '{COLLECTION_NAME_PATENT_NEW}'...")

    if utility.has_collection(COLLECTION_NAME_PATENT_NEW):
        print(f"[MILVUS] Collection '{COLLECTION_NAME_PATENT_NEW}' already exists")
        return Collection(COLLECTION_NAME_PATENT_NEW)

    print(f"[MILVUS] Creating collection '{COLLECTION_NAME_PATENT_NEW}'...")
    fields = [
    ...
    schema = CollectionSchema(fields, description="Patent image embeddings with MinIO URL (Base model)")
    collection = Collection(name=COLLECTION_NAME_PATENT_NEW, schema=schema)
    print(f"[MILVUS] Collection '{COLLECTION_NAME_PATENT_NEW}' created")
...
def main():
    print("[PATENT] ========== Import testImage to new Milvus collection ==========")
    print(f"[PATENT] Collection name: {COLLECTION_NAME_PATENT_NEW}")
    ...
    print(f"[PATENT] Existing files: {len(existing)}")

    # 扫描图片
    all_images = scan_images()
    print(f"[PATENT] Total images: {len(all_images)}")

    # 过滤已存在的
    pending = [img for img in all_images if img not in existing]
    print(f"[PATENT] Pending: {len(pending)}")

    if not pending:
        print("[PATENT] All images already imported!")
        return

    # 延迟导入 DINOv2
    print("[PATENT] Loading DINOv2 model...")
    from app.services.dinov2_base_service import dinov2_base_extractor
    print("[PATENT] Model loaded")
    ...
    print("[PATENT] ========== Import completed ==========")
    print(f"[PATENT] Success: {success_count}, Failed: {fail_count}")
    print(f"[PATENT] Total in collection: {collection.num_entities}")
